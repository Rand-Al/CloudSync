        ...
        modules: {
            tableOfContents: {
                ...
                createMobileTOC: function () {
                    ...
                    // Remove any existing mobile TOC to prevent duplicates
                    var existingMobileTOC =
                        document.querySelector(".mobile-toc-system");
                    if (existingMobileTOC) {
                        utils.log(
                            "Removing existing mobile TOC to prevent conflicts"
                        );
                        existingMobileTOC.parentNode.removeChild(
                            existingMobileTOC
                        );
                    }

                    // Create the main container for mobile TOC system
                    ...
                    // Create slide-up panel for navigation content
                    var navigationPanel = this.createMobileTOCPanel();

                    // Assemble the complete mobile TOC system
                ...
                createFloatingTOCButton: function () {
                    ...
                    // Create main button container with accessibility attributes
                    ...
                    // Create circular progress indicator background
                    var progressRing = document.createElement("div");
                    progressRing.className = "toc-progress-ring";

                    // Create SVG for precise progress visualization
                    var progressSvg = document.createElementNS(
                        "http://www.w3.org/2000/svg",
                        "svg"
                    );
                    progressSvg.setAttribute("width", "60");
                    progressSvg.setAttribute("height", "60");
                    progressSvg.setAttribute("viewBox", "0 0 60 60");
                    progressSvg.className = "progress-circle";
                    ...
                    // Progress circle that fills as user reads
                    ...
                    progressCircle.setAttribute("stroke-dasharray", "163.36"); // 2 * π * 26
                    progressCircle.setAttribute("stroke-dashoffset", "163.36"); // Initially hidden
                    progressCircle.setAttribute(
                        "transform",
                        "rotate(-90 30 30)"
                    ); // Start from top
                    progressCircle.className = "progress-indicator";
                    ...
                    iconContainer.className = "toc-button-icon";

                    // Create icon element with semantic meaning
                    var buttonIcon = document.createElement("i");
                    buttonIcon.className = "fas fa-list-ul";
                    buttonIcon.setAttribute("aria-hidden", "true");

                    // Create text label for screen readers
                    var screenReaderText = document.createElement("span");
                    screenReaderText.className = "sr-only";
                    screenReaderText.textContent = "Table of Contents";

                    // Assemble icon container
                    iconContainer.appendChild(buttonIcon);
                    iconContainer.appendChild(screenReaderText);

                    // Assemble complete button structure
                    ...
                    utils.log(
                        "Floating button created with circular progress indicator"
                    );
